    ...
    return async (data: MapFileActionsToData<FbActionFullUnion>) => {
      switch (data.id) {
        ...
        case CustomActions.ShowInFolder.id: {
          const file = data.state.selectedFiles[0];
          if (!file) break;
          // Ephemeral token handshake: request a short-lived token from
          // the backend, then navigate to /resolve/{token}. If the user
          // is authenticated the server resolves the real path; if not,
...
export const sharefileActions = Object.keys(CustomActions)
  ...
  .filter(
    (action) =>
      ...
      action.id !== CustomActions.ToggleStar.id,
  );
